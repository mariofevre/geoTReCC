...
$query="
    ...
    LEFT JOIN
		$DB.sis_usu_registro ON sis_usu_registro.id = ref_indicadores_indicadores.usu_autor
		
	LEFT JOIN 
		$DB.ref_capasgeo
		ON
		ref_indicadores_indicadores.id_p_ref_capasgeo = ref_capasgeo.id
    WHERE 
        ref_indicadores_indicadores.zz_borrada = '0'
    AND
 	 	ref_indicadores_indicadores.zz_publicada = '1'
	AND
 	  (
	 	  ref_indicadores_indicadores.nombre LIKE '%".$_POST['busqueda']."%'
	 	  OR
	 	  ref_indicadores_indicadores.descripcion LIKE '%".$_POST['busqueda']."%'
	  )
";
...
$Log['tx'][]='filtrando por recorte espacial: '.print_r($c,true);
	foreach($Log['data']['resultados'] as $idind => $datcapa ){	
		...
		//print_r($fila);
		if($fila['count']==0){
			unset($Log['data']['resultados'][$idind]);
		}
	}
}
