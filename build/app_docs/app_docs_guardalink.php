...
if(!isset($_POST['urlLink'])){
	$Log['tx'][]='no fue enviada la imagen en la variable urlLink';
	$Log['res']='err';
	terminar($Log);
}

if(!isset($_POST['linkName'])){
	$Log['tx'][]='no fue enviada la imagen en la variable linkName';
	$Log['res']='err';
	terminar($Log);
}
...
$Log['tx'][]= "Salvando link ".$LinkName." con URL ".$UrlLink;

$query="INSERT INTO $DB.ref_doc_links(
...
//echo $query;
$Log['data']['nid']=$NID;
$Log['data']['nf']=$_POST['nlink'];
$Log['data']['ruta']=$UrlLink;
$Log['tx'][]='completado';
$Log['res']='exito';
terminar($Log);
?>