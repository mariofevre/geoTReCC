...
if(isset($_POST['actualizar_id'])){$Log['data']['actualizar_id']=$_POST['actualizar_id'];}
if(isset($_POST['actualizar_nivel'])){$Log['data']['actualizar_nivel']=$_POST['actualizar_nivel'];}
if(isset($_POST['actualizar_modo'])){$Log['data']['actualizar_modo']=$_POST['actualizar_modo'];}

	$query="
		SELECT `GRAactores`.`id`,
		    `GRAactores`.`nombre`,
		    `GRAactores`.`apellido`,
		    `GRAactores`.`mail`,
		    `GRAactores`.`reporte`
		FROM `paneles`.`GRAactores`
		where  `GRAactores`.`zz_AUTOPANEL`='$PanelI'
	";		
	...
	while($row = $Consulta->fetch_assoc()){
		if($row['id_p_PLAn1']>0){
			$PNCatVals['PLAn1'][$row['id_p_PLAn1']][$row['id_p_PLAcategorias']]=utf8_encode($row['valor']);	
		}	
		if($row['id_p_PLAn2']>0){
			$PNCatVals['PLAn2'][$row['id_p_PLAn2']][$row['id_p_PLAcategorias']]=utf8_encode($row['valor']);	
		}	
		if($row['id_p_PLAn3']>0){
			$PNCatVals['PLAn3'][$row['id_p_PLAn3']][$row['id_p_PLAcategorias']]=utf8_encode($row['valor']);	
		}	
	}	
	...
	while($row = $Consulta->fetch_assoc()){
		...
		if(isset($PNCatVals['PLAn3'][$row['id']])){
			$PN3[$row['id']]['categorias']=$PNCatVals['PLAn3'][$row['id']];					
		}
	...
	while($row = $Consulta->fetch_assoc()){
		...
		if(isset($PNCatVals['PLAn2'][$row['id']])){
			$PN2[$row['id']]['categorias']=$PNCatVals['PLAn2'][$row['id']];					
		}	
	...
	while($row = $Consulta->fetch_assoc()){
		...
		if(isset($PNCatVals['PLAn1'][$row['id']])){
			$PN1[$row['id']]['categorias']=$PNCatVals['PLAn1'][$row['id']];
		}	
	...
	while($row = $Consulta->fetch_assoc()){
		...
		if(isset($img_ext[strtolower($ext)])){
			if(file_exists($row['FI_documento'])){
				$row['mostrar']='si';
			}
		}
	...
	*/
	...
	$query="
		SELECT 
			id, nombre, encabezado, descripcion
		FROM PLAplan
		Where zz_AUTOPANEL='$PanelI' 
	...
	while($row = $Consulta->fetch_assoc()){
		foreach($row as $k => $v){
			$Log['data']['PLA']['general'][$k]=utf8_encode($v);	
		}	
	}	
