...
if(count($_POST['fuentes'])<1){
	...
	FROM 
		$DB.ref_rele_campa
	LEFT JOIN
		$DB.ref_capasgeo ON ref_capasgeo.id = ref_rele_campa.id_p_ref_capasgeo
	WHERE
		...
		if($f['fue']=='nombreUA'||$f['fue']=='nomUA'){
			...
			$arr_input=array();
				...
				$JOINS.="
					...
					LEFT JOIN
						(SELECT id, texto1 as dato FROM $DB.ref_capasgeo_registros WHERE zz_borrada='0' )
						as datotexto".$countt." 
					...
					(SELECT 
						id_p_ref_rele_registros, 
						 CASE 
							...
							ELSE 0
							END as dato
						FROM $DB.ref_rele_registros_datos 
						WHERE 
...
INSERT INTO $DB.ref_capasgeo_registros(
    ...
    FROM    
    	...
    	LEFT JOIN
    	(SELECT id, geom, geom_point, geom_line  FROM $DB.ref_capasgeo_registros WHERE zz_borrada='0' )as registrosgeo ON registrosgeo.id = ref_rele_registros.id_p_ref_capas_registros
