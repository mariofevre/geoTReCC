...
//idco 7: habitaciones. por radio por tipo de vivienda  //EN ESTA ALTERNATIVCA EN REALIDAD NO SE USA PARA ESTE COMPONENTE
	//ref_capasgeo
	$tab_7_hab=$Log['data']['Componentes'][7]['tabla'];
	$reg_7_hab=$Log['data']['Componentes'][7]['id_ref'];

//idco 9: centros clasificados
	//ref_capasgeo
	$tab_9_centros=$Log['data']['Componentes'][9]['tabla'];
	$reg_9_centros=$Log['data']['Componentes'][9]['id_ref'];

//idco 10b: cantidad de viviendas afectadas por departamento
	//ref_capasgeo
	$tab_10b_afec=$Log['data']['Componentes'][21]['tabla'];
	$reg_10b_afec=$Log['data']['Componentes'][21]['id_ref'];
				...
				$Log['tablas'][$fila['id']]['selectquery']="
					...
					LEFT JOIN 
						geogec.ref_capasgeo_registros as lr 
							ON lr.".nombre_a_campo($fila['link_capa_campo_externo'])." = r.".$Log['tablas'][$fila['id']]['campos'][$fila['link_capa_campo_local']]." 
...
$query=$Log['tablas'][$reg_10b_afec]['selectquery'];

$Consulta10b = pg_query($ConecSIG, $query);
if(pg_errormessage($ConecSIG)!=''){
	$Log['tx'][]='error: '.pg_errormessage($ConecSIG);
	$Log['tx'][]='query: '.$query;
	$Log['res']='err';
...
$Log['data']['dtos_10b']=array();
$c=0;
while($fila=pg_fetch_assoc($Consulta10b)){
	$c++;
	$cpo_cod=$Log['tablas'][$Log['data']['Componentes'][21]['id_ref']]['campos'][$Log['data']['Componentes'][21]['campo_t_a']];
	$cpo_porc=$Log['tablas'][$Log['data']['Componentes'][21]['id_ref']]['campos'][$Log['data']['Componentes'][21]['campo_n_a']];
	
	$Log['data']['dtos_10b'][$fila[$cpo_cod]]=$fila[$cpo_porc];
	
}

$Log['tx'][]='datos disponibles de '.$c. ' departamentos';
...
$query=$Log['tablas'][$tab_9_centros]['selectquery'];
...
if($_POST['avance']==0){
	...
	$query = "
		...
		VALUES
			(
				'Polygon', '".$idUsuario."', 
				'radios urbanos y agrupados por centro. con cantidad de habitaciones 1 de 2.',
				
				'".$_POST['codMarco']."', 3857, 0, 1,
				'codigo radio', 'codigo centro', 				
				'habitaciones en casas o casas tipo a', 'habitaciones en casas tipo b', 'habitaciones en departamento', 'habitaciones en rancho','habitaciones en casilla'
			)
		RETURNING ID
	";
	...
	$query = "
		INSERT INTO 
			geogec.ref_proc_instancias_componentes(
				...
				tabla, id_ref, estado, 
				campo_t_a, campo_t_b,
				campo_n_a, campo_n_b, campo_n_c, campo_n_d, campo_n_e
			)
		VALUES (
			'".$_POST['codMarco']."', 
...
foreach($Filas as $n => $fila){
	...
	$query="
			insert into geogec.ref_capasgeo_registros(
				...
				'".($Conteo['Casa Tipo A']+$Conteo['Casa'])."',
				'".$Conteo['Casa Tipo B']."',
				'".$Conteo['Departamento']."',
				'".$Conteo['Rancho']."',
				'".$Conteo['Casilla']."'			
