...
//idco 9: centros clasificados.
	//ref_capasgeo
	$tab_9=$Log['data']['Componente']['tabla'];
	$reg_9=$Log['data']['Componente']['id_ref'];
...
$campocol=$Log['tabla']['campos'][$Log['data']['Componente']['campo_t_c']];
$campofil=$Log['tabla']['campos'][$Log['data']['Componente']['campo_t_d']];

$CamposcolArr=array();
while($fila = pg_fetch_assoc($Consulta)){
	$CamposcolArr[$fila[$campocol]]=0;
	$CamposfilArr[$fila[$campofil]]=array();	
}
...
while($fila = pg_fetch_assoc($Consulta)){
	...
	if(count($ag[$fila[$campofil]])==0){$ag[$fila[$campofil]]=$CamposcolArr;}
	
	$ag[$fila[$campofil]][$fila[$campocol]]++;
	
	//if($fila[$campocol]=='???'){$Log['tx'][]='???:'.$fila['texto2']}
}
...
foreach($ag as $fil => $fildat){
	$htmlstr.='<tr>';
	$htmlstr.='<th>';
		$htmlstr.='/';
		$htmlstr.='</th>';
	foreach($fildat as $col => $valor){
