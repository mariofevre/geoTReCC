...
if($Acc<2){
    $Log['mg'][]=utf8_encode('No cuenta con permisos (nivel 2 vs nivel '.$Acc.') para modificar un indicador. En el marco de investigación código '.$_POST['codMarco']);
    $Log['res']='err';
    terminar($Log);	
}
...
if(pg_num_rows($ConsultaVer)<1){
	$Log['mg'][]='error interno no se encontro el indicador con el id enviado ('.$_POST['id'].') para el marco código: '.$_POST['codMarco'];
	$Log['res']='err';
	terminar($Log);	
}
...
if($f['zz_publicada']=='1'&&$Acc<3){
	$Log['tx'][]='error: esta versión ya fue publicada, no puede eliminarse por quien disponga de un nivel de ingreso inferior a 3.';
	$Log['tx'][]='error: esta versión ya fue publicada, no puede eliminarse por quien disponga de un nivel de ingreso inferior a 3.';
	$Log['res']='err';
	terminar($Log);	
}
...
$query="
	UPDATE  
     ...
     SET 
        zz_borrada = '1',
        zz_borrada_usu = '".$idUsuario."',
        zz_borrada_utime = '".time()."'
    WHERE 
		id = '".$_POST['id']."'
	AND
		zz_borrada = '0'
